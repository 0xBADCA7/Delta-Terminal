...
int graphicsOptionsMenu() {
			...
			[&] {
				...
				if (Event.mouseButton.button == sf::Mouse::Left) {
					if (currentVideoMode > 0)
						...
						currentVideoMode = (unsigned) sf::VideoMode::getFullscreenModes().size() - 1;
				}
				...
				menu[Buttons::Resolution].setString("Resolution : " +
				                                    toString(
						                                    sf::VideoMode::getFullscreenModes()[currentVideoMode].width) +
				                                    "x" +
				                                    toString(
						                                    sf::VideoMode::getFullscreenModes()[currentVideoMode].height) +
				                                    aspectRatio);
			};

	menu.newButton("Fullscreen : " + toString(settings.graphics.fullscreenEnabled))
			.mimicStyles(menu[Buttons::Resolution]) =
			[&] {
				settings.graphics.fullscreenEnabled = !settings.graphics.fullscreenEnabled;
				menu[Buttons::FullscreenToggle].setString(
						"Fullscreen : " + toString(settings.graphics.fullscreenEnabled));
				instanciateWindow();
			};

	menu.newButton("Draw FPS : " + toString(settings.graphics.displayFramerate))
			.mimicStyles(menu[Buttons::Resolution]) =
			[&] {
	...
	menu.newButton("Apply Resolution", 730, 1080 - (Font.getLineSpacing(MENU_FONT_SIZE * 2 + 20)))
			.mimicStyles(menu[Buttons::Resolution]) =
			[&] {
	...
	menu.newButton("Reset", 1700, 1080 - (Font.getLineSpacing(MENU_FONT_SIZE * 2 + 20)))
			.mimicStyles(menu[Buttons::Resolution]) =
			[&] {
				...
				{
					int i = 2;
					double dec = ((double) settings.graphics.VideoMode.width /
					              (double) settings.graphics.VideoMode.height);
					while (mod(round(dec * i) - dec * i) > precision)
						i++;
					aspectRatio = " (" + toString(floor(dec * i)) + ":" + toString(i) + ")";
				}
	...
	menu.newButton("Back", 50, 1080 - (Font.getLineSpacing(MENU_FONT_SIZE * 2 + 20)))
			.mimicStyles(menu[Buttons::Resolution]) = [&] { throw (0); };
