...
Button &Button::mimicStyles(Button &pButton) {
	...
	setOutlineThickness(pButton.getOutlineThickness());
	setFontSize(pButton.getFontSize());
	return *this;
}
...
Button &Button::setFontSize(unsigned fSize) {
	text.setCharacterSize(fSize);
	subText.setCharacterSize(fSize);
	button.setSize(sf::Vector2f(text.getGlobalBounds().width + BUTTONXPADDING,
	                            text.getGlobalBounds().height + BUTTONYPADDING));
	return *this;
}
...
Button &Button::setTextColour(sf::Color colour) {
	...
	subText.setFillColor(colour);
	return *this;
}

Button &Button::setPosition(sf::Vector2f pos) {
	button.setPosition(pos);
	text.setPosition(button.getPosition().x + (BUTTONXPADDING / 2),
	                 (float) (button.getPosition().y + (BUTTONYPADDING * 0.2)));
